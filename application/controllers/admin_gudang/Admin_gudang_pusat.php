		...
		function __construct()
	  	...
	  	function suplier_stock(){
	  		if($this->session->userdata('akses') == 1 && $this->session->userdata('masuk') == true){
	  			...
	  			$x['toko'] = $this->m_pemesanan_new->getAlltoko();
		       	$this->load->view('v_header',$y);
		       	$this->load->view('owner/v_sidebar');
		       	$this->load->view('owner/v_request_stok_suplier',$x);
		    }
		    else{
		       redirect('Login');
		    }
	  	...
	  	function acc_suplier_stock(){
	  		if($this->session->userdata('akses') == 1 && $this->session->userdata('masuk') == true){
		  		...
		  		$tanggal_acc = date("Y-m-d");
		  		$status = 1;
		  		$this->m_request_stock->acc_suplier_stock($barang_nama,$jumlah,$id_toko_dari,$id_toko_ke,$id_admin,$tanggal_acc,$status,$id_request );
		  		echo $this->session->set_flashdata('msg','success');
		       	redirect("Owner/Request_stock");	
	       }else{
		       redirect('Login');
